...
using Fostor.Ginkgo.Authorization.Accounts;
...
namespace Fostor.Ginkgo.Users
{
    ...
    public class UserAppService : AsyncCrudAppService<User, UserDto, long, PagedUserResultRequestDto, CreateUserDto, UserDto>, IUserAppService
    {
        ...
        {
            var users = (await base.GetAll(input)).Items.ToList();
            //改变中文名字的显示方式
        ...
        {
            var user = _repository.FirstOrDefault(x => x.UserName == userName).MapTo<UserDto>();
            if (IsNonAscii(user.Name) || IsNonAscii(user.Surname))
            {
        ...
        public async Task<IdentityResult> ResetUserPassword(string userName, string password)
        {
            ...
            var result = await _userManager.ChangePasswordAsync(user, password);
            return result;
        }
